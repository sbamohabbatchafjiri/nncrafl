...
def load_weights_without_decode_issue(model, weights_path):
    with h5py.File(weights_path, mode='a') as f:  # Change 'r' to 'a' to allow modifications
    ...
    if round_cnt % 2 == 0:
        ...
        if edge_num > len(new_edges):
            ...
            if round_cnt % 2 == 1:
                ...
                # new part to have loop logically correct: Ensure interested_indice doesn't exceed edge_num
                additional_edges_needed = edge_num - len(interested_indice)
                for rare_edge in rare_edge_list:
                    if additional_edges_needed <= 0:
                        break
                    if rare_edge not in new_edges:
    ...
    with open('gradient_info_p', 'w') as f:
        ...
        for idxx in range(len(interested_indice[:])):
            if idxx % 100 == 0:
                ...
                load_weights_without_decode_issue(model, 'model.h5')
                layer_list = [(layer.name, layer) for layer in model.layers]
...
def train(model, seed, label):
    ...
    if seed.shape[0] > 5000:
        model.fit(seed, label, batch_size=int(seed.shape[0] / 50), epochs=300, verbose=1, callbacks=callbacks_list)
    elif 4000 < seed.shape[0] <= 5000:
        model.fit(seed, label, batch_size=int(seed.shape[0] / 10), epochs=200, verbose=1, callbacks=callbacks_list)
    else:
        model.fit(seed, label, steps_per_epoch=50, epochs=100, verbose=1, callbacks=callbacks_list)
